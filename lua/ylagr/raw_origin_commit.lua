...
local function raw_origin_commit(key, env)
   ...
   if context:is_composing() then
    ...
    if string.find(keystr, "asciitilde") then
        ...
        local origin_cand = cand:get_genuine()
        log.error(tostring(origin_cand))
        if origin_cand ~= nil then
            context:clear_previous_segment()
            local commit_text = context:get_commit_text()
            context:clear()
            env.engine:commit_text(commit_text .. origin_cand.text)
